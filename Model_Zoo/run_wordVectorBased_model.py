...
tf.flags.DEFINE_float("init_lr", 0.003, "initial learing rate, default 0.001") 
tf.flags.DEFINE_integer("n_latent", 15 , "the number of lantent factors in Deep Model")
...
tf.flags.DEFINE_integer("num_filters", 50 , "CNN filter num")
...
def generate_batch(batch_index, batch_size, input_data):
    """ generate the batch_data for the input of the model.

    Args:
        batch_size: the number of the samples in each batch
        batch_index: the x-th batch that you choose to return
        input_data: the dataSet that will be precessed

    Returns:
        batch_u_text: shape=[None, u_review_num, review_word_num], the user reviews set of batch data
        batch_i_text: shape=[None, i_review_num, review_word_num], the item reviews set of batch data
        batch_uid: shape=[None,1], users' id
        batch_iid: shape=[None,1], items' id
        batch_y: shape=[None,1], ground-truth ratings
...
def save_attention(model, u_batch, i_batch, uid, iid, y_batch, vocabularyb):
    '''
        ...
        功能：
            仅用于模型MACF。将一个batch的数据送入模型中进行前向传播计算，并返回单词级别的注意力得分word_att和
...
def train(train_dataSet, batch_size):
    ...
    for batch_index in range(batch_num):
        ...
        batch_u_text, batch_i_text, batch_uid, batch_iid, batch_y = \
                generate_batch(batch_index, batch_size, shuffled_data) 
...
def evaluate(test_dataSet, batch_size):
    ...
    for batch_index in range(batch_num):
        batch_u_text_test, batch_i_text_test, batch_uid_test, batch_iid_test, batch_y_test = \
                        generate_batch(batch_index, batch_size, test_dataSet)
...
if __name__ == '__main__':
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            train_op = tf.train.AdamOptimizer(FLAGS['init_lr'], beta1=0.9, beta2=0.999, epsilon=1e-8).minimize(model.loss)
            ...
            for epoch in range(EPOCH):
                ...
                if len(valid_history) > 6:
                    if abs(valid_history[-2][1]-valid_history[-1][1]) < 0.2 and jitter_num < 2 :
            ...
            '''
                =========================================================================
                        For the attentional Model (e.g. DARR, MACF, NARRE), you may
                        need to save the attention weights.
                =========================================================================
            '''
            ...
            '''
                ...
                =========================================================================
            '''
            # model.save(session=sess, model_output_path=Model_Output_Path, model_Name=model_Name, model_Version=model_Version)
            # print('the model',model_Name,'has been saved.')
