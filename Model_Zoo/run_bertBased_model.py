...
import os
import gc
from models.bertBased_Models import DeepCLFM, NCEM
...
# DataSet Path
tf.flags.DEFINE_string("para_data", DIR + "param_bert.dict", "Data parameters and preprocessed data")

# Model Shared Hyperparameters
tf.flags.DEFINE_float("dropout_keep_prob", 0.7, "dropout keep probability")
tf.flags.DEFINE_float("init_lr", 0.002, "initial learing rate, default 0.001") 
tf.flags.DEFINE_integer("n_latent", 15 , "the number of latent factors of the user(item) embedding")
    ...
    for i in range(len(batch_iid)):
        ...
        if len(cur_u_text)>u_review_num: 
            ...
            while len(cur_u_text) < u_review_num: 
                cur_u_text.append([int(0) for _ in range(768)])
        ...
        if len(cur_i_text)>i_review_num: 
            ...
            while len(cur_i_text) < i_review_num:  
                cur_i_text.append([int(0) for _ in range(768)])
...
def train(train_dataSet, batch_size):
    ...
    for batch_index in range(batch_num):
        ...
        # 计算当前batch的rmse和mae，并训练模型（梯度下降更新参数）
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            model = DeepCLFM.build_model(
                        ...
                        attention_dim = FLAGS['attention_dim'] # attention dimension
                    )

            # 初始化模型: NCEM
            # model = NCEM.build_model(
            #             user_num=user_num,  # the total number of users
            #             item_num=item_num,  # the total number of items
            #             u_review_num = u_review_num, # the number of review per user
            #             i_review_num = i_review_num, # the number of review per item
